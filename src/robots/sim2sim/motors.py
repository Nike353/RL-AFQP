...
@dataclass
class MotorCommand:
  desired_position: np.ndarray = np.zeros(12)
  kp: np.ndarray = np.zeros(12)
  desired_velocity: np.ndarray = np.zeros(12)
  kd: np.ndarray = np.zeros(12)
  desired_extra_torque: np.ndarray = np.zeros(12)
...
class MotorGroup:
  ...
  def __init__(self,
    ...
    self._strength_ratios = np.ones((self._num_envs, self._num_motors))
    ...
    self._torque_history.append(
        np.zeros((self._num_envs, self._num_motors)))
    self._torque_output = np.zeros((self._num_envs, self._num_motors))
    self._true_motor_torque = np.zeros((self._num_envs, self._num_motors))
  ...
  def _clip_torques(self, desired_torque: _ARRAY,
    ...
    return np.clip(desired_torque, torque_lb, torque_ub)
  ...
  def convert_to_torque(
    ...
    if motor_control_mode == MotorControlMode.POSITION:
      ...
      kp = self._kps
      desired_velocity = np.zeros((self._num_envs, self._num_motors))
      kd = self._kds
  ...
  def strength_ratios(self, value: _FloatOrArray):
    self._strength_ratios = np.ones(
        self._num_motors) * value
