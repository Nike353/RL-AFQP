...
class mujoco_sim():
    ...
    def __init__(self, **kwargs ):
        ...
        # to make the contacts as "hard" as possible
        for attr in dir(self.model):
            if 'geom_solref' in attr:
                obj = getattr(self.model,attr)
                obj[:] = np.array([-100000, -1000]) # kp, kd of the contact impedence spring
        ...
        if self.sim_params['render']:
            ...
            # force perturbations if applied
    ...
    def init_renderers(self):
        ...
        # any other flags to set
        self.renderer._scene_option.flags[mujoco.mjtVisFlag.mjVIS_PERTFORCE] = 1
    ...
    def get_terrain_height_at(self,pos):
        ...
        # TMP HACK for hardware 
        # BLOCK_START = 1.28
        # BLOCK_END = 1.92 #1.48
        # BLOCK_HEIGHT = 0.1

        # x = pos[0]
        # if x > BLOCK_START and x < BLOCK_END:
        #     # print('x:',x,BLOCK_HEIGHT)
        #     return BLOCK_HEIGHT
        # else:
        #     return 0.0
    ...
    def set_sensordata(self,name,data):
        sensor_id = self.obj_name2id(name,type='sensor')
        start_n = self.model.sensor_adr[sensor_id]
        
        if sensor_id == self.model.nsensor -1:
            self.data.sensordata[start_n:] = data
        else:
            end_n = self.model.sensor_adr[sensor_id+1]
            self.data.sensordata[start_n:end_n] = data
    ...
    def set_sensordata_from_id(self,sensor_id,data):
    ...
    def set_wheels_terrain_friction(self,val=None,fraction_nominal=1):
            ...
            if val is None:
                val = fraction_nominal*self.nominal_model.geom_friction[terrain_id,0]
            self.model.geom_friction[terrain_id,0] = val

            wheel_ids = [self.obj_name2id(name=body_name,type='body') for body_name in ['l_toe_wheel1','l_toe_wheel2','l_toe_wheel3','l_toe_wheel4',
                                                                                        'r_toe_wheel1','r_toe_wheel2','r_toe_wheel3','r_toe_wheel4']]  

            for geom_id,bodyi_id in enumerate(self.model.geom_bodyid):
                if bodyi_id in wheel_ids:
                    self.model.geom_friction[geom_id,0] = val # sliding friction only  
    ...
    def apply_force_on_body( self, body_name, wrench):
        body_id = mujoco.mj_name2id(
                            self.model,
                            getattr(mujoco.mjtObj, 'mjOBJ_BODY'),
                            body_name
                        )
    ...
    def generate_terrain(self):
        ...
        # flush the old terrain
        self.generate_terrain_plane()

        if 'terrain' in self.sim_params.keys():
    ...
    def generate_terrain_hills(
        ...
        for i in range(i_start,i_end): # x axis
            for j in range(j_start,j_end): # y axis  
                self.model.hfield_data[j*self.model.hfield_ncol[0] + i] += amplitude_lim*noise([i/self.model.hfield_nrow[0], j/self.model.hfield_ncol[0]])/ self.hfield_scale

        reshaped_hfield = self.model.hfield_data.reshape(self.model.hfield_nrow[0],self.model.hfield_ncol[0])

    def generate_terrain_steps(
                                self,
                                x_start_lim = 0.5,
                                x_end_lim = 1.0,
                                y_start_lim = -2,
        ...
        patch_length_x = x_end_lim - x_start_lim
        patch_length_y = y_end_lim - y_start_lim

        terrainMap = np.zeros((i_end-i_start,j_end-j_start))

        xNum = int(patch_length_x / step_size)
        yNum = int(patch_length_y / step_size)
        gridWidth = int(step_size / self.hfield_x_index2len) # asssumes self.hfield_x_index2len = self.hfield_y_index2len
        
        for i in range(xNum):
            for j in range(yNum):
                h = np.random.random() * step_height #+0.5
        ...
        for i in range(i_start,i_end): # x axis
            for j in range(j_start,j_end): # y axis
                self.model.hfield_data[j*self.model.hfield_ncol[0] + i] += terrainMap[i-i_start][j-j_start] / self.hfield_scale
    ...
    def generate_terrain_sinusoidal(
        ...
        for i in range(i_start,i_end): # x axis
            for j in range(j_start,j_end): # y axis  
                x_i = ( i - 0.5*self.model.hfield_nrow[0] ) * self.hfield_x_index2len
                self.model.hfield_data[j*self.model.hfield_ncol[0] + i] = amplitude_lim*np.sin( ang_freq_lim*(x_i-x_start_lim) + phase_offset_lim) / self.hfield_scale
